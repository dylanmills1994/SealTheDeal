...
const textLabels: Record<string, string> = {
  hero_eyebrow: "Small intro line",
  hero_headline: "Main homepage headline",
  hero_subheadline: "Homepage intro text",
...
async function isAuthed() { const store = await cookies(); return store.get(COOKIE_NAME)?.value === "1" }
...
async function saveText(formData: FormData) {
  ...
  for (const [id, value] of Object.entries(Object.fromEntries(formData.entries()))) await upsertContent(id, String(value))
  revalidatePath("/"); revalidatePath("/gallery")
  redirect("/admin?success=Website+text+saved")
}
...
async function uploadImage(formData: FormData) {
  ...
  if (!slot || !file) redirect("/admin?error=Missing+image+data")
  if (!ALLOWED_TYPES.includes(file.type) || file.size > MAX_BYTES) redirect("/admin?error=Invalid+file")

  const ext = file.type.split("/")[1] || "jpg"
  const path = `${slot}/${Date.now()}.${ext}`
  const upload = await fetch(`${url}/storage/v1/object/seal-the-deal-images/${path}`, {
    method: "POST",
    headers: { apikey: key, Authorization: `Bearer ${key}`, "Content-Type": file.type, "x-upsert": "true" },
    body: Buffer.from(await file.arrayBuffer()),
  })
  if (!upload.ok) redirect("/admin?error=Upload+failed")

  await fetch(`${url}/rest/v1/site_images`, {
    method: "POST",
    headers: { apikey: key, Authorization: `Bearer ${key}`, "Content-Type": "application/json", Prefer: "resolution=merge-duplicates" },
    body: JSON.stringify({ slot, label: slot, image_url: `${url}/storage/v1/object/public/seal-the-deal-images/${path}`, storage_path: path, alt_text: alt, sort_order: 0, is_active: true }),
  })
...
export default async function AdminPage({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  ...
  if (!authed) return <main className="mx-auto min-h-screen max-w-md px-4 py-14"><div className="rounded-lg border bg-card p-6 shadow-sm"><h1 className="text-2xl font-black uppercase">Seal The Deal Admin</h1><p className="mt-2 text-sm text-muted-foreground">Update website text, images, and gallery content.</p><form action={login} className="mt-5 space-y-3"><input name="password" type="password" className="w-full rounded-md border border-border bg-background px-3 py-2" placeholder="Owner password" required /><button className="w-full rounded-md bg-primary px-4 py-2 text-primary-foreground">Login</button>{params.error && <p className="text-sm text-red-600">{params.error}</p>}</form></div></main>
